<x-app-layout>
      ...
      <div class="border-l-4 border-red-800 px-2">
        <ul class="list-none text-sm ml-0 pl-2">
          @foreach ($all_sections as $one_section)
            <li class="pt-4">
              <a href="{{ route('sections.show', [$project->slug, $one_section->slug]) }}"
                class="text-gray-600 text-base">{{ $one_section->title }}</a>
                ...
                @foreach ($pages as $nav_page)
                  @if ($nav_page->section_id === $one_section->id)
                    <li class="border-l-4 border-yellow-700 pl-2 mb-2">
                      <a href="{{ route('pages.show', $nav_page->slug) }}"
                        class=" text-gray-500 text-sm">{{ $nav_page->title }}</a>
                    </li>
                  @endif
                @endforeach
              </ul>
  ...
  <div
    class="w-full border-t-2 border-green-700 px-2 mb-4 lg:border-none lg:cols-span-3 lg:col-start-2 lg:col-end-5 lg:row-span-4 lg:mr-4 2xl:flex-grow 2xl:mr-0"
    id="main-box">
    <div class="bg-white shadow-2xl rounded p-3 md:w-11/12 md:mx-auto lg:pr-6 2xl:w-full">
      ...
      @endif
      <div>
        <x-breadcrumb link="{{ route('sections.show', [$project->slug, $section->slug]) }}"
          name="{{ $section->title }}" project="{{ $project->name }}"
        ...
        <div class="flex justify-between">
          <div>
            @if ($prev !== null)
              ...
              <a href="{{ route('pages.show', $prev->slug) }}"
                class="text-gray-500 hover:underline hover:text-gray-800 pl-2">{{ $prev->slug }}</a>
            @endif
          </div>
          <div>
            @if ($next !== null)
              <a href="{{ route('pages.show', $next->slug) }}"
                class="float-left text-gray-500 hover:underline hover:text-gray-800 pr-2">{{ $next->slug }}</a>
  ...
  <x-buttons :link="'pages'" :edit="'true'" :delete="'true'" id="{{ $slug->slug }}"
    new="{{ route('pages.create', [$section->project->slug, $section->slug]) }}" :page="'New'">
    ...
    @if ($revision !== null)
      <div class="p-2 ml-2 mr-1 w-1/3 lg:mx-auto lg:p-2 lg:w-10/12 lg:ml-0 lg:mb-2 2xl:w-8/12 2xl:mx-auto">
        <a href="{{ route('revisions.index', [$project->slug, $slug->id]) }}"
          class="text-xs md:text-sm xl:text-base h-12">
          <i class="bi bi-plus-square icons"></i>
          Revision
        </a>
      </div>
